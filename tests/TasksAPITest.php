...
class TasksAPITest extends TestCase
{
    ...
    public function testTaskInDatabaseAreShownByAPI()
    {
        ...
        $this->actingAs($user)->get('/task/' . $task->id)->seeJsonContains(['name' => $task->name, 'done' => $task->done, 'priority' => $task->priority ])
            ->seeStatusCode(200);
    }
    /**
     * Create fake task
    ...
    public function testTasksCanBePostedAndSavedIntoDatabase()
    {
        ...
        $this->actingAs($user)->get('/task')->seeJsonContains(['name' => 'Foobar', 'done' => true, 'priority' => 1])->seeStatusCode(200);
    }
    /**
    ...
    public function testTasksCanBeUpdatedAndSeeChangesInDatabase()
    {
        ...
        $this->actingAs($user)->get('/task')->seeJsonContains(['name' => 'Learn Laravel', 'done' => false , 'priority' => 3])->seeStatusCode(200);
    }
    /**
     ...
     * @return void
     */
    public function testTasksReturnLoginSecurity()
    {
